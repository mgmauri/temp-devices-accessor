...
def test_set_valid_target_temperature_by_evk(silicon_thermal_service, evk_name: str):
    import random

    target_temperature = random.uniform(-5, 100)  # FIXME add config
    silicon_thermal_service.set_target_temperature_by_evk(evk_name, target_temperature)
    assert (
        silicon_thermal_service.get_target_temperature_by_evk(evk_name)
        == target_temperature
    )


@pytest.mark.parametrize("evk_name", silicon_thermals_evk_names())
def test_reached_temperature_by_evk(silicon_thermal_service, evk_name: str):
    import math
    import time
